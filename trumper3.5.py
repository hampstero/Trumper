...
def word_after_four(prefix,suffix_map_4):
	return suffix_map_4.get(prefix)

def stupid_talk(suffix_map_1,suffix_map_2,suffix_map_3,suffix_map_4,corpus):
	speech = []
	...
	prefix = speech[-3] + ' ' + speech[-2] + ' ' + speech[-1]
	word_choices = word_after_triple(prefix,suffix_map_3)

	while (len(word_choices) == 0):
		index = random.randint(0,len(corpus)-3)
		prefix = corpus[index] + ' ' + corpus[index+1] + ' ' + corpus[index+2]
		word_choices = word_after_triple(prefix,suffix_map_3)

	word = random.choice(word_choices)
	speech.append(word)	

	prefix = speech[-4] + ' ' + speech[-3] + ' ' + speech[-2] + ' ' + speech[-1]
	word_choices = word_after_four(prefix,suffix_map_4)
	
	while (len(word_choices) == 0):

		index = random.randint(0,len(corpus)-4)
		prefix = corpus[index] + ' ' + corpus[index+1] + ' ' + corpus[index+2] + ' ' + corpus[index+3]
		word_choices = word_after_four(prefix,suffix_map_4)
...
dict4 = dictionary4(corpus)
speech = stupid_talk(dict1, dict2, dict3, dict4, corpus)
print(*speech, sep=' ')